...
describe('canRespond', () => {
	...
	it('allows requests when all limits are within bounds', async () => {
		...
		const result = await canRespond(config, TEST_IP, TEST_SESSION, 0);
		expect(result).toEqual({ allowed: true });
	});

	// -- Session cap --------------------------------------------------------

	it('blocks when session message count equals the cap', async () => {
		const config = defaultConfig(kv, { sessionCap: 10 });
		const result = await canRespond(config, TEST_IP, TEST_SESSION, 10);
		expect(result.allowed).toBe(false);
		expect(result.reason).toContain('Session message limit');
	});

	it('blocks when session message count exceeds the cap', async () => {
		const config = defaultConfig(kv, { sessionCap: 10 });
		const result = await canRespond(config, TEST_IP, TEST_SESSION, 15);
		expect(result.allowed).toBe(false);
	});

	it('allows when session message count is below the cap', async () => {
		const config = defaultConfig(kv, { sessionCap: 10 });
		const result = await canRespond(config, TEST_IP, TEST_SESSION, 9);
	...
	it('reports session cap before checking KV-based limits', async () => {
		const config = defaultConfig(kv, { sessionCap: 5, monthlyCap: 50, rateLimitPerHour: 30 });
		// Both KV limits are also exceeded
		kv._store.set(monthlySpendKey(), '100');
		kv._store.set(rateLimitKey(TEST_IP), '100');

		const result = await canRespond(config, TEST_IP, TEST_SESSION, 5);
		expect(result.allowed).toBe(false);
		expect(result.reason).toContain('Session message limit');
	});
});

 ...
 * evaluates WINDOW_APP_MAP = synthWindowAppMap() over MANIFESTS at module load, so
 * reaching it before manifests finishes reads an undefined MANIFESTS. This module
 * imports nothing at runtime (the ResolvedWindow import is type-only, erased), so
 * it breaks the cycle.
 */

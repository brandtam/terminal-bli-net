...
- You can always hang up, like it's 1987: `ALT-H` drops the line from anywhere in a call (it's on the new ProComm-style status bar), and the Hayes `+++` escape — guard time and all — works too, with `ATH`, `ATO`, and `AT` answered the way a Smartmodem would. Every trunk request now carries a 15-second deadline, so a line that stops answering degrades to LOCAL MODE instead of stranding the caller mid-screen.
